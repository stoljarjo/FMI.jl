...
function main()
    ...
    if github_token != "" && cross_check_repo_name != "" && cross_check_repo_user != ""
        withenv(
            "GIT_SSH_COMMAND" => isnothing(github_token) ? "ssh" : "ssh -i $pkey_filename -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null"
        ) do
            # run(Cmd(`ssh-keyscan -t rsa github.com >> ~/.ssh/known_hosts`))
            run(
